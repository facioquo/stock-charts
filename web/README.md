...
React + Vite + React Router frontend for the Stock Charts demo — the migration target replacing the Angular app in [`client/`](../client) per the org React standard (ADR-0001).

> **Migration in progress.** This is the **foundation slice**. The app builds, tests pass, and the chart page renders default indicators (via backup data when the backend is offline). The settings/indicator-picker UI, full SCSS theme port, Playwright E2E, and Angular-toolchain removal land in stacked follow-up slices. `client/` remains the deployed app until parity is reached.
