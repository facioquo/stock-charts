 ...
 * Creates over 1000 business days of historical data from 2016-2019
 */
function generateBackupQuotes(): Quote[] {
  const quotes: Quote[] = [];
  const startDate = new Date("2016-01-04"); // Start on a Monday
  let currentPrice = 200.0; // Starting price for QQQ in early 2016
  
  // Generate approximately 4 years of business days (over 1000 quotes)
  for (let dayOffset = 0; quotes.length < 1100; dayOffset++) {
    ...
    const quote = generateDailyQuote(currentDate, currentPrice, quotes.length);
    quotes.push(quote);
 ...
 */
function generateDailyQuote(date: Date, previousClose: number, dayIndex: number): Quote {
  ...
  const dailyVolatility = (Math.random() - 0.5) * 0.04; // ±2% typical daily range
  const extraVolatility = Math.random() < 0.1 ? (Math.random() - 0.5) * 0.06 : 0; // 10% chance of ±3% extra volatility
  
  const open = basePrice * volatilityFactor * (1 + (Math.random() - 0.5) * 0.01);
  const close = open * (1 + dailyVolatility + extraVolatility);
  
  // High and low with realistic spreads
  const maxOC = Math.max(open, close);
  const minOC = Math.min(open, close);
  const high = maxOC * (1 + Math.random() * 0.015); // Up to 1.5% above max of O/C
  const low = minOC * (1 - Math.random() * 0.015);  // Up to 1.5% below min of O/C
  
  // Realistic volume based on price movement
  const priceChangePercent = Math.abs((close - open) / open);
  const baseVolume = 50000000 + (Math.random() * 100000000); // 50M-150M base
...
function isMarketHoliday(date: Date): boolean {
  ...
  // Good Friday (varies, but typically in March/April) - simplified
  if ((month === 3 || month === 4) && date.getDay() === 5 && Math.random() < 0.2) return true;
